...
### Fixed
- `GET /_events/stream` connections were forcibly closed after approximately 10 seconds in production, making the EventStream feature (shipped in A274) unusable for any extended session. Root cause: `App.defaultWriteTimeout = 10 * time.Second` is wired into the shared `http.Server.WriteTimeout`, which per Go's standard library is a fixed deadline set once when request headers are read and is never reset by intermediate `Flush()` calls. Event-stream connections complete their initial `200` header response and first flush well within the 10-second window, but then idle until the first `{"type":"ping"}` heartbeat fires at the 25-second mark — by then the fixed deadline has already expired, forcing the Go runtime to close the connection before any event is written. `eventstream.go`'s `newEventStreamHandler` now uses `http.NewResponseController(w)` (Go 1.20+) to refresh a per-write deadline via `SetWriteDeadline(time.Now().Add(2 * eventStreamHeartbeat))` immediately before every write (both event-delivery and heartbeat branches), creating a rolling ~50-second deadline at the default 25-second heartbeat interval. Two alternatives were considered and rejected: (1) a separate dedicated `http.Server` with `WriteTimeout: 0` — rejected because it breaks the single-mux pattern every other route in the codebase follows and removes write-deadline protection from the entire route rather than scoping it to the one deliberately unbounded connection; (2) disabling the deadline entirely with a single `SetWriteDeadline(time.Time{})` call at connection start — rejected because it leaves a genuinely wedged write (e.g. a half-dead TCP peer that never sends FIN/RST) completely unbounded for the connection's lifetime, inconsistent with the per-token concurrent-connection cap added in A275 to bound the same class of unbounded-resource risk on this route. For a `ResponseWriter` that does not support write deadlines (returns `http.ErrNotSupported` — true only for custom test implementations, never a real HTTP/1.1+ connection), the handler logs a warning and still attempts the write rather than aborting. Confirmed live by devops via three independent tests, including a direct loopback test to `127.0.0.1:8081` bypassing the reverse proxy entirely, ruling out any proxy/network/TLS cause. Regression test `TestEventStreamHandler_SurvivesWriteTimeout` reproduces the exact bug using `httptest.NewUnstartedServer` to set a 100ms `WriteTimeout` before starting the server; verified to fail against pre-fix code (connection closes after 3 of 5 expected lines) and pass with the fix. Coverage: 96.3% package-wide, 97.6% for `newEventStreamHandler`. No exported symbol. (A276)
...
### Fixed
- `doc.go`: godoc cross-reference `[Signal]` corrected to `[LifecycleEvent]` — since Amendment A183 renamed `Signal` to `LifecycleEvent` and reused `Signal` for an unrelated orchestration content type, this link silently pointed at the wrong type.
- `ai.go`: the `LLMsTemplateData` godoc example referenced a template function name (`forge_llms_entries`) that no longer exists; corrected to `smeldr_llms_entries`. The actual `/llms-full.txt` generated footer text ("Generated by Forge on...") is now "Generated by Smeldr on..." — this is real output served to AI crawlers, not a comment.
- `smeldr_test.go`: a doc comment claimed `/_health` includes a `"forge"` version key; the real key is `"core"`. Comment corrected; `TestApp_health_forgeVersion` renamed `TestApp_health_coreVersion` to match what it actually asserts.
- `auth.go`/`smeldr.go`: four `WARN` stderr messages used a `forge:` prefix inconsistent with the `smeldr:` prefix used everywhere else in the same functions; corrected to `smeldr:`.

### Changed
- Completed the Forge→Smeldr rename across the remaining doc comments, log strings, and unexported identifiers in `core` (found during an awesome-go submission readiness review): `doc.go`'s package doc comment, `storage.go`, `benchmarks_test.go`, `example/docs/main.go`'s full embedded doc-page prose (including a struct-tag code sample that used the dead `forge:"required"` key instead of the real `smeldr:"required"` — A107 — silently producing zero validation if copy-pasted), and prose/log-prefix references across `smeldr.go`, `module.go`, `mcp.go`, `context.go`, `errors.go`, `head.go`, `nav.go`, `node.go`, `outbound.go`, `relations.go`, `sitemap.go`, `social.go`, `templatedata.go`, `templatehelpers.go`, `schema.go`, `markdown.go`, `example/blog/main.go`, and `example/api/main.go`. Unexported identifiers renamed for consistency (`forgeVersions`→`smeldrVersions`, `mcpParseForgeTag`→`mcpParseSmeldrTag`, and ~15 `TestForgeXxx`→`TestSmeldrXxx` test names) — no exported symbols affected. `core/pgx` submodule's Go package renamed `forgepgx`→`pgx` (see `pgx/CHANGELOG.md`, v0.2.0). Preserved deliberately: `migrate.go`'s legacy `forge_*` table-rename literals, the `X-Forge-*` HTTP headers (T86/T87 deprecation window), and `mcp.WithForgeFallback()`. (A223)
