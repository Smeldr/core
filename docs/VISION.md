...
This document captures the long-term vision for Smeldr and Smeldr Cloud.
...
Smeldr is built differently. From day one, every architectural decision has
...
Smeldr is the typed, persistent state layer that AI agents operate on.
...
Smeldr is the state. The AI agent is the workflow engine.

This is a precise distinction. Temporal and LangChain orchestrate AI
within traditional software infrastructure. Smeldr inverts this: the AI
agent orchestrates, and Smeldr provides the typed, stateful substrate it
operates on. MCP is not a bolt-on — it is the protocol by which agents
drive state transitions on Smeldr entities. Smeldr and workflow engines
are complements, not competitors.

Every property that makes Smeldr useful for content — lifecycle
...
Smeldr is the product. Authored AI is the methodology it was built with —
...
Smeldr embodies this methodology. Its API naming, file structure, and
...
## Why this is achievable with Smeldr
...
defines its own schema via Go struct tags and the `Head()` method. Smeldr
...
endpoints, and gzip-compressed AI responses are part of v1.0.0. A Smeldr
site is already optimised for AI consumption before MCP is added.

**Smeldr is a structured contract layer between humans and AI.** The role
...
Smeldr's existing architecture maps cleanly onto MCP primitives:

| Smeldr concept | MCP concept |
|---|---|
...
MCP is not a new system sitting beside Smeldr. It is a thin transport layer
...
### What an AI assistant can do via Smeldr MCP
...
**Smeldr Cloud — commercial**
"Give me a site in 10 minutes." No Go, no deployment, no server.
smeldr-admin handles provisioning, dashboard, and multi-site management.
smeldr-admin is closed source. The customer never writes Go.

Cloud architecture: process-per-tenant. One Smeldr instance and one SQLite
...
### Phase 3 — Smeldr Cloud private beta (current focus)

Invitation-only. smeldr-admin provisions one Smeldr instance per customer
(process-per-tenant, SQLite). smeldr-admin has a proper database from day
one. Per-site tokens are an internal smeldr-admin detail — never exposed to
end users.
...
`smeldr.dev/core` and `smeldr.dev/mcp` are licensed under the GNU Affero General
Public License v3 (AGPL).

AGPL means: the source code is open and free to use, modify, and distribute.
If you use Smeldr to provide a hosted service to others, you must release your
modifications under the same license.

For individual developers, open source projects, and companies building their
own sites with Smeldr: AGPL imposes no meaningful restriction. You can use
Smeldr freely.

### smeldr-admin — closed source

`smeldr-admin` is a commercial component of Smeldr Cloud. It is not
part of the open source framework and is not available separately.

### Future — Commercial license (AGPL exemption)

When Smeldr Cloud launches commercially, a commercial license will be available
for organisations that want to use Smeldr as the basis of a hosted service
...
Smeldr launched under MIT. No external contributors exist as of March 2026,
so relicensing to AGPL requires no coordination. The CLA signed by future
contributors grants smeldr the right to issue commercial licenses without
...
Smeldr Cloud is not a competitor to Vercel, Netlify, or Railway. Those are
general-purpose deployment platforms. Smeldr Cloud is a content platform
