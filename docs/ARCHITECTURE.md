...
│                     App.RegisterFlow(StateFlow) error — idempotent startup upsert
│                     (INSERT OR IGNORE + SELECT id); validateFlowItems (unexported) —
│                     SQLite-only unknown-state check against the type's table;
│                     validateTransition (unexported) — checks (flow_id, from, to) in
│                     smeldr_transitions; falls back to default flow; ErrConflict on
│                     disallowed; identity + nil DB + non-SQLite → nil
│                     (Amendments A175, A176, T23)
