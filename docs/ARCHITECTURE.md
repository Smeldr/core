...
| 2026-08-18 | A279 (T223, Level 2, smeldr/core half): new `sweep_run.go` — `SweepRunRecord`, `SweepRunStore` interface, `sqlSweepRunStore`, `NewSweepRunStore(DB)`, `CreateSweepRunTable(DB)`; `Append`/`Last`/`List` — one immutable record per completed scheduled detector run, modeled directly on `audit.go`'s `AuditRecord`/`AuditStore` shape. Closes T223: a clean structural sweep previously logged one Debug line and persisted nothing (`smeldr/agent/sweep.go:51-60`), indistinguishable from no sweep ever having run. Settled "where does the run record live" against D38/D51: not `Run` (D38 — a heavier claim/lease/worktree object for M3 headless coding sessions, category mismatch) and not `Finding` (D51 — a persisted, detector-owned, subject-keyed record of one detected condition; a run record is one row per *execution*, ordered by time, existing whether or not anything was found). Breaking signature change, confirmed in plan review rather than taken unilaterally per the source task's own instruction: `RelationStore.SweepStructural`/`App.SweepStructural` (`relations.go`, `smeldr.go`) and `App.DrainEvalQueue` (`state.go`) all gain a leading `walked int` return — the total items examined this run, without which `flagged=0, skipped=0` cannot be told apart from "nothing to check" versus "checked everything, found nothing," the exact ambiguity T223 exists to close. No HTTP or MCP surface by design (`Last`/`List` are for Go-level staleness derivation only, same reasoning as `/_logs`); a public read surface is left to a future task if one turns out to need it. No wiring into `agent.NewSweepScheduler`'s own signature: the base `smeldr.dev/agent` package is MIT and deliberately carries zero dependency on `smeldr.dev/core` (verified directly — `NewEvalQueueScheduler` already takes an anonymous interface rather than `*smeldr.App` for exactly this reason; only `smeldr.dev/agent/flow`, a separate AGPL sub-package, imports core), so a `SweepRunStore` parameter there would break that boundary — recording a run belongs in the caller's own wrapping closure instead, documented as a godoc usage example on `SweepRunStore`. Actually scheduling `SweepStructural`/`DrainEvalQueue` on the live `process.smeldr.dev` instance remains explicitly out of scope, same reasoning A240 already applied to the eval-queue drain: no observability, no silent automated authority. Companion `smeldr.dev/agent` half (A280, this same cycle): `SweepFunc`'s type and `NewEvalQueueScheduler`'s anonymous interface both widen to match; the scheduler's own Debug/Info log-level branch now also considers `walked` (`walked > 0` alone is informative, even with `flagged=0, skipped=0`). Every pre-existing `RelationStore.SweepStructural`/`App.SweepStructural` test (`relations_sweep_test.go`, `smeldr_target_checker_test.go`) and `App.DrainEvalQueue` test (`state_test.go`, `state_transition_item_test.go`) updated for the new arity, each asserting the correct `walked` count for its own scenario (including that a scan-error row still counts as walked, since walked increments before `Scan` is attempted). 10 new tests for `sweep_run.go` (round-trip, not-found, DB-error and scan-error paths for `Append`/`Last`/`List`, table idempotency, detector isolation, newest-first ordering with `limit`). No exported symbols removed. Coverage: 96.3% package-wide; `sweep_run.go`'s own functions 100% except `NewSweepRunStore`'s trivial constructor line (also 100%); `RelationStore.SweepStructural` 87.1% — four fatal-DB-error branches (initial query error, initial scan error, `rows.Err()`, per-edge UPDATE error) were already uncovered before this change and remain so, unrelated to the `walked` addition, named rather than chased. MINOR bump (new exported API surface plus a breaking signature change with no known external importers, matching this project's own precedent of MINOR rather than MAJOR for exactly this situation — see A264/D50-era precedent cited in the plan). v1.74.0 → v1.75.0. |
| 2026-08-20 | A288 (T210, Level 2): `scanDest` (`storage.go`) gains `nullTimeScanner`, handling a `**time.Time` scan destination — a nullable `*time.Time` struct field's own address, unmatched by the existing `*time.Time` case (A200). `Node.ScheduledAt` (embedded in every compiled `Module[T]` type) previously failed a hard Scan error on any real `SQLRepo`-backed round-trip with a non-nil value; now works. `RelationEdge.ValidAt`/`InvalidAt` checked and confirmed unaffected (own hand-written `sql.NullTime` scan). `Run.AcknowledgedAt`'s own `time.Time`-not-`*time.Time` type, a deliberate prior workaround for this exact bug, flagged as a stale artifact worth its own future cleanup Task, not touched here. No exported symbol changed. PATCH bump: v1.76.2 → v1.76.3. |
| 2026-08-20 | A287 (T87, Level 2): `httpDeliver` (`outbound.go`) no longer dual-emits the legacy `X-Forge-Signature`/`X-Forge-Timestamp`/`X-Forge-Event`/`X-Forge-Delivery` webhook headers — `X-Smeldr-*` only. Extends D53's reasoning from core's exported Go API to wire-protocol compatibility, per Peter's own explicit 2026-08-15 go-ahead. `smeldr.dev/mcp` (removes legacy `forge://` MCP resource URI scheme) and `smeldr.dev/cli` (removes legacy `FORGE_*` env vars/`.forge-cli.env`) ship the same task's other two repos. No exported symbol changed. PATCH bump: v1.76.1 → v1.76.2. |
| 2026-08-20 | A286 (Level 1): `provenance.go`'s `List` had a dead `n++` after its last optional filter branch (`f.ActorID`) — `staticcheck` SA4006, caught closing out the T237 tag/release cascade (same bug class independently found and fixed the same session in `smeldr.dev/mcp`'s `node_tools.go`). Removed; no behaviour change, no exported symbol changed. PATCH bump: v1.76.0 → v1.76.1. |
| 2026-08-20 | A285 (`sweep-structural-wired-on-schedule`, Level 1): `App.SweepStructural` wired on a real cron schedule in `example/server` — new `EnableStructuralSweep`/`ENABLE_STRUCTURAL_SWEEP` (requires `EnableRelations` explicitly, fails loudly rather than silently no-op) and `StructuralSweepSchedule`/`STRUCTURAL_SWEEP_SCHEDULE` (default `"0 * * * *"`, hourly) via `agent.NewSweepScheduler`; each run recorded through `SweepRunStore` (T223/A279) via the documented wrapping-closure pattern (`sweep_run.go`). `smeldr.dev/agent` bumped v0.7.1 → v0.8.0 in `example/server/go.mod` (v0.7.1 predates A280's `walked`-widening). All four of A240's original blockers verified closed for this detector specifically (observability via `SweepRunRecord`; Signal dispatch via `a.emitSignal`→`a.dispatchBus`, the same path webhooks/event-stream use; authority — not applicable, mutates a relation edge's `invalid_at`, never a governed content `Status`). `App.DrainEvalQueue`'s own scheduling deliberately **not** wired this cycle — two of A240's four blockers remain open by current, explicit design (`state.go:1088-1103`: signal dispatch and cache invalidation for automated `Decision` re-evaluation transitions) — reversing that is its own decision with its own consequences, left for a dedicated future Task rather than bundled in here. `buildApp` falls back to the hourly default itself when `StructuralSweepSchedule` is empty (not only via `parseConfig`'s own `envOr` default), matching `agent.NewEvalQueueScheduler`'s own established fallback pattern — caught by a real test failure during implementation, not assumed. 3 new subtests in `TestServerToggles`. No `smeldr` package file touched — `example/server`-only. No version bump, no tag, matching A245/A246/A282's own precedent. |
| 2026-08-20 | A283 (T159, Level 2): `BuildContextPacket`'s Published-only status gate (`context_packet.go`) had a 0% pass rate for its entire domain since A214 shipped (over a month) — `Status` never holds lifecycle vocabulary for the five orchestration anchor types (`goal`/`decision`/`amendment`/`task`/`signal`), only each type's own governed-flow state name (`"backlog"` for `Task`, `"open"` for `Goal`, etc., D56). Both the anchor gate (`:233`) and the linked-item gate (`:335`) removed entirely, matching D47's "alive means the row exists, no status consulted" applied to packet visibility instead of structural-sweep liveness. **Held twice before shipping**: first (2026-08-16) because the route is unauthenticated by original design (A214, an isolated demo instance never deployed) but is mounted on the real, live instance today whenever `ENABLE_RELATIONS && ENABLE_ORCHESTRATION` — removing the gate as designed would have made every orchestration item publicly readable with a guessable slug; second (2026-08-20) because Peter's own 2026-08-16 access-model answer sat unaddressed in the plan file for four days, the text on record still the pre-hold no-auth version. Fix ships both halves together: `GET /packet/{type}/{slug}` now requires `Editor` role via the identical inline auth pattern `newAuditHandler`/`newLogsHandler`/`newEventStreamHandler` already use (`auth.authenticate(r)` then `user.HasRole(Editor)`, hierarchical — `Admin` passes too) — no shared helper extracted this cycle, argued rather than defaulted (would touch three already-shipped unrelated handlers; deferred to a real fifth caller). `example/server` gains its own explicit `EnableContextPacket`/`ENABLE_CONTEXT_PACKET` flag alongside the existing two prerequisites, defaulting off, matching this binary's one-flag-per-feature convention rather than piggybacking on two unrelated flags' side effect. 12 new/updated tests: two existing draft-status tests inverted (`_draftAnchor`, `_draftLinkedItemExcluded`→`_draftLinkedItemIncluded`); two new tests build items with real flow-state values directly instead of the shared `insertTest*` helpers' hardcoded `Published`, reproducing the actual reported bug; four new auth tests (401/403/200-Admin/200-end-to-end); three existing handler tests gain a token; `example/server` gains a flag-combination test. Shared `insertTest*` fixtures' hardcoded `Published` — the reason this bug went undetected for a month — flagged, not fixed (shared across many unrelated tests, separate larger piece of work). No exported Go symbol changed. Coverage: 96.3% package-wide; `ContextPacketHandler` 92.6% — one pre-existing, unrelated gap (a valid custom `?depth=2` at the HTTP-handler level was never covered before this change either), named not chased. `go test -race ./...` clean, `golangci-lint` zero findings. PATCH bump, matching A247/A253/A261/A281's own precedent: v1.75.1 → v1.75.2. |
| 2026-08-18 | A282 (T217, Level 1): two new CI checks close a real gap. `.github/workflows/ci.yml` gained a matrix `examples` job building all four `example/*` directories on every push/PR (`GOWORK=off`) — nothing in CI previously built any of them, the exact blind spot A245's month-long unbuildable `example/server` traced to; deliberately validates every example against core's current checked-out HEAD (every example replaces `smeldr.dev/core => ../..`), not against any published core version — strictly the right and stronger scope for catching a breaking core change on the same commit that introduces it, confirmed as the intended scope rather than assumed. New scheduled `.github/workflows/pin-currency.yml` (weekly cron + `workflow_dispatch`) runs a new standalone Go tool, `scripts/checkpins` (own `go.mod`, `golang.org/x/mod/modfile` beyond stdlib — CI-tooling-only, never part of the `smeldr` package's own zero-dependency graph), comparing every *direct*, non-`replace`d `smeldr.dev/*` pin against the real module proxy — a passing build alone cannot catch a stale-but-compiling pin (the class T266's mcp-pin-behind-tools gap was). Indirect requires skipped by design: T217's own motivating incidents are all about a direct pin missing a reachable capability, a different and lower-signal risk class than indirect drift `go mod tidy` already manages on its own. **Running the tool for real during grounding corrected two plan-time assumptions rather than confirming them**: `example/server`'s `smeldr.dev/agent v0.7.1` is *not* stale — `v0.8.0` was pushed to `main` this same session (T223) but never tagged, so the proxy still correctly reports `v0.7.1` as latest, and bumping to it would have pinned a version with no installable tag; `example/blog`'s `smeldr.dev/oauth v0.2.0` (an indirect dependency) *was* found stale, outside the plan's own original two-pin scope, resolved by the indirect-skip design decision rather than bumped by hand. Fixed: `example/blog`'s `smeldr.dev/mcp v1.30.0` → `v1.31.1` via `go get`+`go mod tidy` (never hand-edited); `example/server`'s `agent` pin left untouched per the correction above. 9 new tests in `scripts/checkpins/main_test.go`; `checkModule` (the parse+compare logic) 100% covered, `main`/`goListLatestVersion` (CLI/network glue) untested by design — a standalone tooling module, not gated at the `smeldr` package's own 96%. No `smeldr` package code touched. No version bump, no tag (config/tooling-only, matching A245/A246's own precedent exactly). |
| 2026-08-18 | A281 (T203, Level 2): `RoleStore.Grant`/`Revoke` (`governance.go`) and `TokenStore.Revoke` (`auth.go`) previously wrote zero `ProvenanceRecord` entries — `Grant`/`Revoke` wrote only to `GovernanceAuditStore` (a full before/after diff), and `TokenStore.Revoke` wrote to neither store at all, a bare `UPDATE smeldr_tokens SET revoked_at = $1 WHERE id = $2` and nothing else, confirmed by direct read. Verified against D44's own boundary before building, per the task's explicit instruction: D44's sentence ("governance audit answers who was given the right to act, ProvenanceRecord answers who then acted; a gap in one is never argued away by pointing at the other") is about not *substituting* one record for the other, not about forbidding both from describing the same event — `GovernanceAuditStore` remains the sole authoritative full-diff record, untouched by this change; `ProvenanceRecord` adds a second, narrower entry compatible with the same `SubjectProvenance` read mechanism every other subject already uses, closing the real gap T202 found (a design mockup's mapping table assumed admin objects already flowed through provenance, and they didn't). Also verified T243's authority-gating model: neither a `RoleGrant` nor a `Token` has (or should have) a `StateFlow`, so `transitionIsGated` returns `false` unconditionally for these subjects and every entry renders fully attributed, never gated down — matching, not contradicting, `GovernanceAuditStore`'s own already-transparent posture for the same class of action. Fix: `RoleStore`/`TokenStore` each gain an unexported `provenanceStore ProvenanceStore` field + `setProvenanceStore` setter, wired at `App.Handler()` time exactly like `RelationStore`'s own existing wiring (`smeldr.go`); `Grant`/`Revoke`/`TokenStore.Revoke` each call `recordProvenance` after their existing write succeeds, reusing `RelationStore.recordAssertProvenance`'s exact shape (ctx.(Context) type assertion for actor recovery, `actorKindFor` for `ActorKind` — deliberately not `RoleStore.actorTokenID`, a token fingerprint in a different identity space than `ProvenanceRecord.ActorID`'s user/job/agent contract). New `SubjectType` values `"RoleGrant"`/`"Token"` — the first non-`Node`, non-`RelationEdge` subjects `ProvenanceRecord` has carried — and `Verb` reuse (`"assert"`/`"invalidate"`, mirroring `RelationEdge`'s own create/remove pair) rather than growing the enum for a structurally identical shape. Fail-open throughout, matching every existing `recordProvenance` call site: an `Append` failure never fails the grant/revoke/token-revoke operation itself. 13 new tests across `governance_test.go` (8) and `auth_test.go` (5): `Grant`/`Revoke`/`TokenStore.Revoke` each record the correct `SubjectType`/`Verb`/actor, fail-open on nil store and on `Append` failure, a plain `context.Context` leaves actor fields empty without panicking, plus two end-to-end tests driving the real `App.Handler()` wiring path rather than calling `setProvenanceStore` directly. No exported symbols removed; no HTTP or MCP surface added — `RoleGrant`/`Token` become queryable through the existing `SubjectProvenance` function only. Coverage: 96.3% package-wide; `setProvenanceStore` (both) and `recordGrantProvenance` 100%; `Grant`/`Revoke` unchanged from their pre-existing baseline. PATCH bump (no new exported symbol — `provenanceStore`/`setProvenanceStore` are both unexported — but a real, tag-worthy consumer-observable behaviour change: `SubjectProvenance` now returns entries for two newly-reachable `SubjectType` values it never produced before, matching A275's own precedent for this exact class): v1.75.0 → v1.75.1. |
| 2026-09-14 | A309 (D63/D64, Level 2): `Transition.RequiredRole string` (exact-role-name match via `RoleStore.RoleGranted`, always a zero `AuthTarget{}`) renamed to `Transition.RequiredOperation string` (operation match via `RoleStore.Authorized`, same model every MCP tool already uses) — real behavior change, not a relabel. `validateTransition` gains `itemID string` (carried into a real `AuthTarget{TypeName, ID}`, previously always zero) and `rels *RelationStore` (accepted, not yet consulted — reserved for D62's future `RequiredRelation` existence-check, avoiding a second breaking signature change later). Two of the three call sites (`Module[T]`, `DynamicTypeRepo`) had no `*RelationStore` access at all before this Amendment — new `Module[T].relationStore`/`setRelationStore` + `App.relationStoreModules` wiring (mirrors the existing `roleStore`/`governanceModules` pattern exactly) and `DynamicTypeRepo.WithRelations` (mirrors `WithGovernance`, auto-wired by `App.DynamicContentRepo`) close that gap; `App.TransitionItem` already had `a.relationStore`. `orchDecisionFlow()`'s four `RequiredRole: "admin"` gates migrate to `RequiredOperation: "approve"` — already reserved in `governance-model.md` §4/§6 for exactly this shape of act and already present in the default `admin` role's seeded `Operations` list since A188 (checked via `git log -L`, not assumed), so no existing admin token is locked out. `TransitionOption.RequiredRole`, `drainAuthorizationGate`'s and `recordAuthorizationRequiredSignal`'s `requiredRole` locals, and `Signal.RequiredRole` (the exported struct field backing `DrainEvalQueue`'s auto-generated authorization-required Signal) all renamed to `RequiredOperation` for consistency — folded in as the same mechanical rename, not a separate decision, since they all read the same underlying value. **`Signal.RequiredOperation`'s JSON key and DB column deliberately kept as `required_role`** (unlike the Go field name) — both are live wire contracts (`smeldr.dev/mcp`'s `create_signal`/`get_signal`/`list_signals` tools, and `process.smeldr.dev`'s own running orchestration instance) that this Amendment does not rename, avoiding a second, live-system-breaking change beyond what D64 itself scoped; found during implementation, not in the original plan. Real, disclosed consumer-visible consequence: the auto-generated Signal's `receiver` column (set from the same value) now carries an operation word (e.g. `"approve"`) instead of a role name (e.g. `"admin"`) for the two migrated `pending-re-evaluation` gates — verified nothing in core resolves `receiver` against `RoleStore` for routing, so this is a value-content change with no internal breakage, but any external consumer expecting a role name there should be told. No DB schema/column change anywhere — the Go field names were always decoupled from their SQL column names (raw SQL, no `db`-tag scanning for these specific reads/writes), so renaming the exported Go symbols needed no `Ensure*Column` migration. New `Module[T]`/`DynamicTypeRepo`/`App.Handler` plumbing tests (`smeldr_test.go`, `state_test.go`) verify the wiring directly; `orchestration_test.go`'s existing gate-definition test and the `state_test.go`/`state_transition_item_test.go` `RequiredRole`-fixture tests all updated for the new field name and operation-based semantics (role-name fixtures like `"editor"` replaced with real operation words like `"manage"` where the fixture's actual point was exercising `Authorized`, not just passing an opaque string through). D53 precedent applied for the breaking change (re-verified, not assumed from D53's own five-years-stale importer count): MINOR bump with `BREAKING` stated plainly, no v2 — confirmed no external importer via the module proxy at commit time. Coverage 96.4% package-wide, `go test -race ./...` clean, `golangci-lint` zero findings. `docs/REFERENCE.md`'s `Transition`/`TransitionOption`/`Signal` structured-fields/`ValidTransitions`/`SubjectProvenance` sections all updated to the current field name and a corrected worked example. v1.86.0 → v1.87.0. |
| 2026-09-14 | A310 (D63/D64's own §5 follow-up, `decision-governance-model.md` §5, Level 2): rule-type stewardship — "route on the union of two lookups, not one address," surface→executor (unchanged, a process convention) and rule type→standing authority (new). Modeled as a real, scoped role grant (D43), not a dedicated table: `RoleGrant.ScopeStatic` already accepts any `"type:id"` pattern with no validation that `type` names a real registered content type (confirmed by reading `Authorized`'s own `ScopeStatic` matching directly), so a synthetic `AuthTarget{TypeName: "RuleType", ID: <name>}` grants standing authority over a rule-type domain via the already-shipped `define_role`/`grant_role` MCP tools — zero new schema, zero new core code for the grant side. New `RoleStore.StewardedRuleTypes(ctx, tokenID) ([]string, error)` (every `"RuleType:<name>"` static-scope pattern on a grant whose role holds the new `"steward"` operation — reserved in `governance-model.md`'s own vocabulary table by architect in the same review pass, mirroring D64 item 3's process) and `RoleStore.StewardshipInbox(ctx, tokenID) (*StewardshipInbox, error)` (the pull-discoverability half of §5: every `Decision`/`Rule`/`AuthorityStub` whose own `RuleType` matches a stewarded domain, via the existing generic `Query[T]` — `storage.go`) are the only new code. Classification ("which domain a new item falls under") needed nothing new: `Decision.RuleType`/`Rule.RuleType`/`AuthorityStub.RuleType` (A303/A304) are already ordinary, ungated fields settable by any Author+ role. Explicitly not built, per §5's own text: push notification (a later enhancement) and an MCP tool exposing `StewardshipInbox` (core-only Task; the tool is a `smeldr.dev/mcp` follow-up). 13 new tests in `governance_test.go` covering both functions' every branch (empty/single/multiple grants, wrong operation excluded, non-static scope excluded, non-`RuleType:` prefix excluded, malformed JSON skipped, query errors on both functions, cross-type matches, no-match, fail-closed on a mid-query DB error). `docs/REFERENCE.md` gains a new "Rule-type stewardship" section under "Roles & auth." Coverage 96.3% package-wide (`StewardedRuleTypes`/`StewardshipInbox` in the low 90s, matching `Authorized`/`RoleGranted`'s own established baseline for the same class of DB-failure leaves, not chased). No exported symbol removed, purely additive. MINOR bump: v1.87.0 → v1.88.0. |
| 2026-09-14 | A311 (Level 1): `seedToolPolicies` (`governance.go`) gains one new row, `{"get_stewardship_inbox", "read"}`, alongside the three existing `get_goal_context`/`list_type_tools`/`get_sweep_run` rows — same reason as all three: a framework tool with no backing `MCPModule` is invisible to the mcp-side verb-derived policy fallback (D48), so it needs an explicit seeded row or every governance-enabled caller is silently forbidden the moment `smeldr.dev/mcp`'s companion `get_stewardship_inbox` tool ships (same gap A298 already closed once for `get_sweep_run`, found again in passing while planning that tool, not rediscovered the hard way). `TestRoleStore_ToolPolicy_OrchestrationDiscoveryTools` extended to cover the new row. `docs/REFERENCE.md`'s rule-type stewardship section (A310) and `AGENTS.md`'s MCP tool table both updated to name the tool now that its policy row exists. No exported symbol changed. PATCH bump (consumer-observable: a specific tool that would otherwise be silently forbidden now works, matching A298's own bump shape for the identical class of fix): v1.88.0 → v1.88.1. |
...
| 2026-09-14 | A312 (`decision-governance-model.md` §4, Level 2): Check — an enforced precondition on a Decision's `proposed→ratified` transition. New `check.go`: `CheckRecord`/`CheckStore` (`Append`/`Last`/`List`, `NewCheckStore` sqlite/pgx-agnostic implementation over `smeldr.DB`), `CreateCheckTable`, `App.Check(store CheckStore) *App` (nil by default — no behaviour change for an app that never calls it). `queryAuthorityCandidates` is the cheap-retrieval half (two indexed `SELECT`s against active `Rule`/unconverted `AuthorityStub` rows sharing `RuleType`, decision-governance-model.md §4's own cost-control requirement); `RunAuthorityCheck` is the full check — candidate selection (most-recently-`PublishedAt` wins, the same convention `SweepRunStore.Last` already uses), sentence composition ("this touches %q, an existing %s rule" found / "no conflicting authority found" not found — naming existence, not rank, since a same-RuleType match cannot honestly claim to outrank its own match), then `store.Append`. Fail-open throughout, matching every other advisory mechanism in this codebase (`SweepStructural`, async triggers): a `queryAuthorityCandidates` failure returns no finding; a `store.Append` failure still returns the computed record alongside the error; neither ever blocks the ratification itself. Wired into **both** paths a Decision can be ratified from — `Module.updateHandler` (`runDecisionAuthorityCheck`, decodes the real `*Decision`) and `App.TransitionItem`/`TransitionItemWithReason` (`runDecisionAuthorityCheckByID`, fetches `RuleType` by raw SQL since `TransitionItem` never decodes a typed struct) — deliberately covering both from the start rather than repeating the exact gap D34's `authorizeDecisionScope` left (`updateHandler` only) that A307 later had to close as its own incident. `example/server/main.go` gains `ENABLE_AUTHORITY`/`ENABLE_CHECK` flags (Authority's own boot wiring — `CreateAuthorityTables`/`RegisterAuthorityTypes`/`RegisterAuthorityRelationKinds` — had zero non-test call sites anywhere before this commit, the same fault class as A307, closed here as this Task's own Step 1 rather than raised separately since Check cannot be demonstrated end-to-end without it). 100% coverage on every new `check.go` function including `App.Check` and both wiring functions' error branches (`RunAuthorityCheck` failing, not just `store.Append` failing in isolation); package-wide 96.3%. `go test -race ./...` clean, `golangci-lint` zero findings. `docs/REFERENCE.md` gains a new "Authority Check" section under the rule-type stewardship section (A310). A real, separately-tracked gap found while planning this Task — D34's `authorizeDecisionScope` still wired into `updateHandler` only, never into `App.TransitionItem` — was raised to the architect as its own Signal rather than folded into this Amendment's scope; tracked as its own Task, not fixed here. MINOR bump (new exported `App.Check`/`CheckStore`/`CheckRecord`/`RunAuthorityCheck`/`CreateCheckTable`/`NewCheckStore`): v1.88.1 → v1.89.0. |
| 2026-09-19 | A325 (`decision-governance-model.md` §6, Level 2): Propagate — declared-tension aggregation and rule-type-weighted severity. Two `Decision` fields (`TensionRuleID`/`TensionReason`, `orchestration.go`), same empty-means-undeclared convention as `RuleType`/`Reversibility` (A304) — `EnsureDecisionTensionColumns` migrates a pre-existing database, wired into `example/server/main.go`'s boot path in this same commit (the `EnsureDecisionClassificationColumns`/`EnsureAmendmentBodyColumn`/`EnsureStateLockedColumn` incident's own standing checklist item). New `tension.go`: `DefaultTensionThreshold` (package `var`, default 3 — not a DB-configurable per-rule-type table; this project has no real usage data yet to justify one, decision-governance-model's own §13 names the whole model as validated against a single-operator organization only, and `RuleType`/`Reversibility` themselves shipped fields-only first for the identical reason), `recordDeclaredTension` (reuses D51's `FindingStore.Record` directly — genuinely "not new machinery," the design doc's own framing: counts ratified Decisions sharing a `TensionRuleID`, records a `Finding{Detector:"declared-tension", SubjectType:"Rule", Provenance:"asserted"}` at or past the threshold), `runDeclaredTensionAggregation`/`runDeclaredTensionAggregationByID` wired into **both** `Module.updateHandler` and `App.TransitionItem` (`state.go`), mirroring A312's own dual-wiring exactly, same fail-open/advisory posture. `App`/`Module[T]` gain `findingStoreModules`/`setFindingStore` plumbing (`smeldr.go`/`module.go`), the same shape `checkStoreModules`/`setCheckStore` already established for `CheckStore` — `App.Findings` was wired at App level only before this (D51/A322); this is the first consumer needing it inside a `Module[T]`. New `severity.go`: `Severity` (small closed ordinal — `SeverityLow`/`Medium`/`High`/`Critical`, not an open numeric score, matching `Reversibility`/`RuleTypeRank`'s own precedent: the named consumer is a cloud UI severity badge, which needs a small finite set of visual treatments, not per-consumer bucketing logic), `SeverityOf(ctx, db, rs, anchorType, anchorID, ruleType, kind, direction, maxDepth)` — the higher of `fanoutSeverity` (buckets `Reachability`'s own real total item count; thresholds are a first cut, not derived from usage data, isolated to one function for easy future adjustment) and `ruleTypeFloorSeverity` (buckets `ruleType`'s own `RuleTypeRank` position *relative to the full currently-registered ordering* — an absolute rank number carries no fixed meaning across organizations, only its position relative to the registered set does; empty or unregistered `ruleType` returns `SeverityLow`, never silently treated as the highest tier) — a floor only, never a substitute for real fan-out, per §3/§6's own explicit reasoning (a locally-scoped decision can carry enormous real consequence; a foundational one can have negligible current fan-out simply because nothing has been built against it yet). No MCP tool in this Task's own scope, matching `Reachability`'s own T194/T196 core-first split. 29 new tests across `tension_test.go`/`severity_test.go` (threshold crossing below/at/above, unratified decisions excluded from the count, both dual-wiring paths, fail-open on a count-query error and a `Record` error, fan-out bucket boundaries, rule-type relative-position bucketing including the single-registered-rank edge case, `SeverityOf`'s fan-out-wins/floor-wins/both-agree/`Reachability`-error cases). Package coverage 96.2% (gate 96.0%); two low-value branches named rather than chased — `severityRank`'s unknown-value default (already exercised transitively via every bucket test, direct coverage of the literal switch arm is a diminishing-returns pursuit) and `ruleTypeFloorSeverity`'s `ruleTypeRankCount` query-error path (would require a purpose-built failing-DB double for one already-fail-open branch whose sibling `RuleTypeRank` error path is already covered). `go test -race ./...` clean, `golangci-lint` zero findings. Two genuinely open design questions (the threshold's configurability model; the severity scale's shape) were raised explicitly to the architect before implementation rather than decided unilaterally — both answered and recorded in this Amendment's own live body. MINOR bump (new exported `Decision.TensionRuleID`/`TensionReason`, `DefaultTensionThreshold`, `EnsureDecisionTensionColumns`, `Severity` and its constants, `SeverityOf`): v1.90.2 → v1.91.0. |
| 2026-09-21 | A344 (`drain-eval-queue-wired-on-schedule`, Level 1): `App.DrainEvalQueue` wired on a real cron schedule in `example/server` — new `EnableEvalQueueDrain`/`ENABLE_EVAL_QUEUE_DRAIN` (requires `EnableOrchestration` explicitly, fails loudly rather than silently no-op) and `EvalQueueDrainSchedule`/`EVAL_QUEUE_DRAIN_SCHEDULE` (default `"*/5 * * * *"`, matching `agent.NewEvalQueueScheduler`'s own built-in default) via `agent.NewSweepScheduler` directly (not `NewEvalQueueScheduler` — that helper has no hook for the `SweepRunStore.Append` wrapping this needs, confirmed directly against `sweep.go:87-111` before writing any code); each run recorded through `SweepRunStore` with `Detector: "eval-queue"` — already named as an anticipated example in `sweep_run.go`'s own doc comment, not a new convention. This is the dedicated future Task A285 deferred to: T211/A258 already closed two of A240's original four blockers for `DrainEvalQueue` specifically (`drainAuthorizationGate` — a role-gated transition is never applied automatically, an `authorization-required` Signal is recorded instead; `recordProvenance` on every successful automated transition); the other two (Signal dispatch, cache invalidation for an automated `Decision` re-evaluation transition) stay unbuilt, not closed by new code — accepted as safe for the one live target instead, on this Task's own reasoning: `DrainEvalQueue`'s only live target today is D40's Decision re-evaluation flow, whose two transitions (`pending-re-evaluation→ratified`, `pending-re-evaluation→superseded`) are exactly the four operation-gated transitions A309/A337 confirmed this same session, so a scheduled drain will almost always land on the safe record-Signal-and-wait branch, not the silent-auto-transition branch A240's original concern was about. Not a reversal of A285/T211's own text — this Amendment records the new, narrower reasoning that makes the deferral safe to close for this one target, rather than silently overriding what A285 said at the time it was true. 3 new subtests in `TestServerToggles` (`off/noEvalQueueDrain`, `on/evalQueueDrainRequiresOrchestration`, `on/evalQueueDrain`), mirroring A285's own three exactly. No `smeldr` package file touched — `example/server`-only. No version bump, no tag, matching A285/A245/A246/A282's own precedent. `go test -race ./...` clean (root and `example/server`), `golangci-lint` zero findings, package coverage unchanged at 96.2%. |
| 2026-09-23 | A347 (D78 follow-up, `01a0ce3a`, Level 2): `last_actor` — persist who performed a state transition. `state.go`'s `TransitionItemWithReason` and `dynamic.go`'s `setStatus`/`ScheduleContent` already extracted `actorID` from the caller's context for the authorization check but discarded it before the UPDATE; all three now persist it. `LastActor string` (json/db `last_actor`) added directly to `Signal`/`Task`/`Decision`/`Amendment`/`Goal`/`Run` (`orchestration.go`) and to `DynamicNode` (`blocks.go`) — **not** to `Node`: `SQLRepo`'s `dbFields` (`storage.go`) builds every SELECT/INSERT/UPDATE column list by reflecting the Go struct, and every content type embeds `Node`, so a `Node`-level field would require a `last_actor` column on every compiled type's table, including third-party module tables this framework does not control — verified and rejected before writing any code, not assumed safe. `TransitionItemWithReason`'s own UPDATE is generic across any compiled type's table (`resolveItemTable`), so its three-column write fails open to the original two-column form on `isNoSuchColumn` (mirrors the existing `isNoSuchTable` precedent) — a table this framework doesn't control keeps behaving exactly as before. `DrainEvalQueue`'s automated transitions record `last_actor="drain-eval-queue"`, reusing the identity its own `recordProvenance` call already uses for the same mechanism, rather than a second name. New `EnsureLastActorColumns(ctx, DB) error` (`orchestration.go`) migrates the six orchestration tables plus `smeldr_dynamic_content` on a pre-existing database — unlike every other `Ensure*` function in this file, it probes each table's existence first and skips one that isn't there, since the seven tables are each gated by their own independent boot-path feature flag (`EnableOrchestration`, `EnableBlocks`/`EnableDynamicContent`); wired unconditionally into `example/server/main.go`'s boot path in this same commit (the `EnsureDecisionClassificationColumns`/`EnsureAmendmentBodyColumn`/`EnsureStateLockedColumn` incident's own standing checklist item). Exposed via `get_task`/`list_tasks`/`get_decision`/etc. and `get_content`/`list_content` automatically — `Module[T].MCPGet`/`MCPList` (`module.go`) return the fetched struct directly, so no `smeldr/mcp` change was needed; confirmed by reading the actual return path before assuming it, not inferred. 12 new tests (`TestEnsureLastActorColumns_*`, `TestCreateOrchestrationTables_LastActorColumn`, `TestApp_TransitionItemWithReason_PersistsLastActor`/`_LastActorEmptyForSystemCaller`, `TestApp_TransitionItem_FailsOpenWhenLastActorColumnMissing`, `TestDynamicTypeRepo_{ScheduleContent,SetStatus}_PersistsLastActor`, `TestDynamicTypeRepo_SetStatus_LastActorEmptyForPlainContext`, `TestDrainEvalQueue_SetsLastActorToDrainEvalQueue`); `gatedItemFixture` (`state_test.go`) gained the column so its own call-count fault-injection tests (`TestDrainEvalQueue_DeleteFails`) keep exercising exactly the calls they name, rather than silently shifting by one on the fail-open retry. Package coverage 96.2% (gate 96.0%). `go test -race ./...` clean, `golangci-lint` zero findings. MINOR bump (new exported `LastActor` fields, new exported `EnsureLastActorColumns`, new consumer-visible `last_actor` key on every orchestration/dynamic-content MCP read response): v1.94.0 → v1.95.0. |
| 2026-09-26 | A353 (G-REL-MCP1 batch, 01a0dc73, Level 2): `orchTaskFlow()` (`orchestration.go`) gains two transitions: `{From: "backlog", To: "deferred"}` and `{From: "backlog", To: "resolved", RequiredReason: true}`. Found live 2026-09-26: `Task`'s own state flow had no legal exit from `backlog` except to `active` — a Task superseded before anyone claimed it (`01a0d827-3`) had no legitimate terminal state to land in; the only workaround was leaving it in `backlog` with priority dropped and a manual "SUPERSEDED" notice prepended to its own description. Purely additive — no existing transition removed or altered, `resolved` and `deferred` were already real states, only their reachability from `backlog` was missing. `orchTaskFlow`'s own godoc updated (the "reachable from active/waiting-plan/plan-reviewing" list now reads "backlog/active/waiting-plan/plan-reviewing"). `TestTaskFlow_ResolvedReachableFromThreeStates` renamed to `TestTaskFlow_ResolvedReachableFromFourStates` (`backlog` moved from `notWant` to `want`); new `TestTaskFlow_DeferredReachableFromActiveAndBacklog` (no prior dedicated test existed for `deferred`'s own reachability at all). Package coverage 96.2% (gate 96.0%, unchanged), `golangci-lint` zero findings. No new exported symbol (`orchTaskFlow` is unexported; `Transition`/`State` are pre-existing types, only their data changed) but a real, disclosed consumer-visible behaviour change — `transition_item`/`get_valid_transitions` now report two transitions they didn't before. PATCH bump, matching A311's own precedent for this class of change: v1.95.0 → v1.95.1. |
...
│                     LastActor string (json/db "last_actor") added to all six orchestration
│                     types — persists the actorID TransitionItemWithReason already extracted
│                     for the authorization check but previously discarded (D78, Amendment A347)
...
│                     Decision gains TensionRuleID/TensionReason (decision-governance-model §6):
│                     the Rule this Decision declares tension against, and the required reason —
│                     empty means undeclared, same convention as RuleType/Reversibility. Not
│                     code-enforced as required-together yet (a future Check-precondition task,
│                     §4). EnsureDecisionTensionColumns(ctx, DB) error mirrors
│                     EnsureDecisionClassificationColumns's own A304 pattern exactly (Amendment A325)
...
│                     GET /packet/{type}/{slug}[?depth=], Editor role required via bearer auth
│                     (same contract as App.Audit's GET /_audit — auth resolved once at
│                     registration time via a.cfg.Auth, defaulting to BearerHMAC);
│                     anchorTypeEntry, anchorTypeTable, packetFetchItem, packetFieldsFromItem,
│                     packetItemURL (unexported) (Amendment A214, T145);
│                     both the anchor-status gate and the linked-item-status gate removed —
│                     Status never holds lifecycle vocabulary for these 5 orchestration types
│                     (D56), so a Published-only check had a 0% pass rate for its entire domain
...
│                     (Detector:"declared-tension") at or past the threshold. Wired into both
│                     Module.updateHandler and App.TransitionItem (state.go), mirroring check.go's
│                     A312 dual-wiring. Fail-open, no-op with no FindingStore configured (A325)
├── severity.go         Severity (ordinal: Low/Medium/High/Critical), SeverityOf(ctx, db, rs,
│                     anchorType, anchorID, ruleType, kind, direction, maxDepth) — decision-
│                     governance §6: the higher of Reachability's own real fan-out and ruleType's
│                     own RuleTypeRank position (relative to the full registered ordering), the
│                     rank acting only as a floor, never a substitute for fan-out (A325)
├── blocks.go          DynamicNode (embeds Node; TypeName, Fields json.RawMessage, LastActor
│                     string — D78, Amendment A347) + Head(),
...
│                     (T49 Step 4, A193);
│                     RoleStore.provenanceStore field + setProvenanceStore (unexported), wired at
│                     App.Handler() time when both App.Provenance and App.Governance are configured —
...
│                     TokenStore.provenanceStore field + setProvenanceStore (unexported), wired at
│                     App.Handler() time when both App.Provenance and Config.TokenStore are
│                     configured — Revoke records a ProvenanceRecord (SubjectType "Token", Verb
│                     "invalidate") after the UPDATE succeeds; TokenStore previously had zero
│                     provenance/audit awareness of any kind (T203, A281)
