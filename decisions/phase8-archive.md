# Smeldr — Phase 8 Archive (A139–A150)

Archived 2026-06-15. Sourced from decisions/recent.md.
...
**`SeedBlockTypeSchemas(db)`** — `INSERT OR IGNORE` seeds 16 canonical block types. Idempotent.

**`ValidateBlockFields(schema, fields)`** — rejects unknown fields and missing required fields. `ErrNotFound` on schema lookup → pass-through (backwards compat). Nil/empty fields treated as `{}`.
...
**`handleTypedTool(ctx, name, args)`** — `args` IS the fields dict; marshals, validates against schema, saves `DynamicNode`. `typeName = strings.TrimPrefix(name, "create_")`.

### Version

core v1.38.0 (minor) · mcp v1.20.0 (minor — new tools: `get_content_type_schema`, `list_content_type_schemas`, `create_<type_name>` × 16).
...
1. SQLite probe — `SELECT COUNT(*) FROM sqlite_master`.
2. Per-pair source check — skip if source absent.
3. Per-pair destination check — if destination exists, log `slog.Warn` and skip.
4. Single transaction — all renames in `BeginTx`; rollback on error.
5. `slog.Info` per rename.
...
| social | 8 `forge_social_*` tables → `smeldr_social_*` |
...
Statement coverage for `smeldr.dev/core` was at 95.9%. T102/N60 targets ≥97%.
Fifty-one uncovered statement blocks across twelve source files were covered in a
new `coverage_test.go` file (41 tests). `codecov.yml` added: project gate 96%/0.5%,
patch gate disabled.

Result: **97.1%** statement coverage.

### Version

Test-only; no exported symbols or behaviour changed. `codecov.yml` added.
