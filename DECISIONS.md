...
| A151 | T104 Inc 1: schema generalisation + content-type registry. `kind` column on `smeldr_content_type_schemas` + `MigrateSchemaKindColumn`; `SchemaField.Role`/`.Relation`; `ValidateFields` (renamed from `ValidateBlockFields`, back-compat alias, type checking, url relative-path, duplicate-role rejection); `ContentTypeRegistry`+`TypeDescriptor`+`LookupByPrefix`+`RegisterPrefix` (registry.go); `App.TypeRegistry()`; `Content()` registration with same-TypeName alias guard; `idx_dynamic_content_type_status` on `smeldr_dynamic_content`. 21 new tests. core v1.39.0. | Agreed | 2026-06-15 |
| A152 | T96: ContentList block resolver. `ContentLister` interface + `Module[T].listPublished` (type-erased JSON round-trip, `var row` avoids receiver shadow); `TypeDescriptor.Fetch` wired at `App.Content()` time; `BlockRenderer.registry` + `ctx` on `renderBlock`; `content_list` special case: `LookupByPrefix(ContentType)` → `Fetch(ctx, contentListOpts)` → `data["Items"]`; `contentListOpts` maps `Limit`→`PerPage`, `Page`→`Page`, `SortField` (published_at/created_at/title)→`OrderBy`, `SortDir` desc→`Desc`; `slog.Warn` on empty ContentType, unknown ContentType, nil Fetch, FilterTags non-empty, fetch error; graceful skips. 9 new tests. core v1.40.0. | Agreed | 2026-06-15 |
| A153 | T104 Inc 2: dynamic content substrate. `PluralSnake` + `ValidateSchemaDef` (exported); `DynamicTypeRepo` (CRUD: CreateDraft/GetBySlug/GetByID/List/UpdateFields/SetStatus); `App.DefineContentType` (saves schema, registers TypeDescriptor+prefix); `App.DynamicContentRepo` (typed repo for registered content types); `loadDynamicTypes` (boot-time DB load, idempotent); `App.ServeDynamicContent` (panics without DB; registers public `GET /{slug}` + `GET /{seg1}/{seg2}` + 5 admin `/_content/*` routes). 90+ new tests in dynamic_test.go + dynamic_app_test.go; also covers `TypeRegistry.All()` + `RegisterPrefix` idempotent path. Coverage: 96.0%. core v1.41.0. | Agreed | 2026-06-16 |
| A154 | T104 Inc 2 patch: operator-controlled URL routing. Root cause: `GET /{seg1}/{seg2}` catch-all conflicted with Go 1.22 mux literal routes. Fix: `ContentTypeSchema.URLPrefix string` (operator-set; empty = admin-only, no public routes); `MigrateURLPrefixColumn` migration; admin routes changed from `{prefix}` → `{type}` (type_name); `ContentList.ContentType` block field now stores type_name not URL prefix; `rebuildDynamicSitemap` goroutine on `SetStatus`; `GET /{seg1}/{seg2}` catch-all removed. 17 new tests + 4 existing ContentList tests fixed. Coverage: 96.0%. core v1.41.1. | Agreed | 2026-06-16 |
| A155 | Security: SSRF fix in outbound webhook transport. `ssrfSafeDialContext()` — unexported `DialContext` func that resolves DNS before connecting and rejects loopback, RFC1918, link-local, unspecified, CGNAT (100.64.0.0/10), IPv6 unique-local (fc00::/7) at dial time (defeats DNS rebinding). Wired into `outboundClient` transport. `workerPool.Enqueue` rejects non-https `target_url`. False `outboundClient` comment corrected. 2 new tests (`TestSSRFProtection` 8 IPs, `TestEnqueueHTTPSValidation`); 2 existing delivery tests updated to swap client. No exported symbols. Coverage: 96.0%. core v1.41.2. | Agreed | 2026-06-17 |
| A157 | T72 PageMeta — per-path SEO override layer. `PageMeta` struct (Path, MetaTitle, Description, OGImage); `PageMetaStore` backed by `smeldr.DB`; `NewPageMetaStore`, `CreatePageMetaTable` (smeldr_page_meta DDL, idempotent); `Set` (INSERT OR REPLACE), `Get` (zero PageMeta on miss, nil error), `Delete` (no-op on miss), `List` (ordered by path). `App.PageMeta(store) *App` wires store; `App.GetPageMeta(ctx, path) Head` for custom handlers. `Handler()` push loop injects store into templateModules via `setPageMetaStore`. `renderListHTML` auto-populates data.Head from store when no ListHeadFunc is set; ListHeadFunc takes priority. 14 new tests. Coverage: 96.0%. core v1.42.0. | Agreed | 2026-06-19 |
