...
type Signal struct {
	...
	// SubjectType and SubjectID name the item this Signal is about, when
	// the Signal was generated by the system rather than written by a
	// human — e.g. [recordAuthorizationRequiredSignal] names the item
	// whose transition was blocked. Empty for an ordinary human-authored
	// Signal. Mirrors [ProvenanceRecord]'s own SubjectType/SubjectID
	// vocabulary for the same concept, not a parallel name (A296).
	SubjectType string `json:"subject_type" db:"subject_type"`
	SubjectID   string `json:"subject_id" db:"subject_id"`
	// FromState and ToState name the blocked transition, when this Signal
	// concerns one. Empty otherwise. Mirrors [ProvenanceRecord]'s own
	// FromState/ToState.
...
// EnsureOrchestrationSignalColumns adds [Signal]'s subject_type/subject_id/
// from_state/to_state/required_role columns to smeldr_signals on
// pre-existing SQLite databases that predate A296. Fresh installs already
// have these columns via [CreateOrchestrationTables]'s own CREATE TABLE
// statement; this only upgrades a database created before A296. Idempotent
// — safe to call on every boot. Uses [EnsureColumn] directly (T246's own
// established pattern for a column an application's own type declares),
// not a hand-written PRAGMA/ALTER pair.
...
func RegisterOrchestrationRelationKinds(ctx context.Context, store *RelationStore) error {
	kinds := []RelationKindDef{
		...
		{
			// contradicts is symmetric — "A contradicts B" reads identically
			// to "B contradicts A", unlike contains/part-of, so no
			// ReverseLabel is set. Directional: false is honest metadata
			// (T251/D59's own kind-registration precedent); no validation
			// logic in this package currently branches on Directional, so
			// this does not by itself make a query symmetric — flagged in
			// A296, not silently relied upon.
			TypeName:    "contradicts",
			Label:       "Contradicts",
			Mode:        "asserted",
			Directional: false,
			TypePairs:   json.RawMessage(`[{"source_type":"Decision","target_type":"Decision"}]`),
		...
		{
			TypeName:     "investigates",
			Label:        "Investigates",
			ReverseLabel: "Investigated By",
			Mode:         "asserted",
			Directional:  true,
